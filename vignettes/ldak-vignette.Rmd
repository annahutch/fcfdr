...
TODO list dependencies (give version etc.): 
- LDAK 
- plink
...
Note that we simplify the subsequent steps by omitting the sex chromosome data.

Subsequent processing steps depend on the data being in the `plink`-compatible `bed`, `bim`, and `fam` file formats. We use `plink` to convert the `vcf` files to these formats.
