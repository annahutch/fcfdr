...
Both functions require 3 parameters to be specified:
...
* `indep_index` in `flexible_cfdr()` or `chr` in `binary_cfdr()`

    * `indep_index`: The indices of the subset of independent SNPs, which can be readily found using various software such as LDAK or PLINK.
    * `chr`: The chromosome that each SNP resides (vector of per-SNP chromosome values)
